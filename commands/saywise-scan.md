---
description: Scan recent Claude Code and Codex CLI conversations and draft Saywise stories from any that are worth it.
---
...
If there is a new conversation worth turning into a Saywise story, draft it; if nothing clears the bar, draft nothing. Either way, update the state file and report what happened. Interactive runs create the drafts as private Suggested Drafts via the Saywise MCP server — the user accepts each on their profile before anything publishes, and an explicit "don't submit" stops submission; unattended runs never call MCP tools and only write files.
